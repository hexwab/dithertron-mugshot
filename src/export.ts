...
function dataURItoArrayBuffer(url) {
    var binary = atob(url.split(',')[1]), array = [];
    for(var i = 0; i < binary.length; i++) array.push(binary.charCodeAt(i));
    return new Uint8Array(array);
}

function getImageFormat() {
    const png = dataURItoArrayBuffer(dest.toDataURL("image/png")),
          shift = 55440,
...
function downloadImageFormat() {
	...
	reader.addEventListener('loadend', () => {
	    // reader.result contains the contents of blob as a typed array
            var blob = new Blob([getImageFormat()], { type: "image/png" });
...
function toTTL(rgb) {
    ...
    let g=(rgb>>8) & 0xff;
    let b=(rgb>>16) & 0xff;
    return 7-(r>0x7f ? 1:0)-((g>0x7f)?2:0)-((b>0x7f)?4:0);
}

function doSend() {
    var data = getNativeFormatData();
    const pal = dithertron.lastPixels.pal;
    let outpal = new Uint8Array(16);
    var ula;
    switch (dithertron.settings.id) {
	case 'bbcmicro.mode0.overscan':
	    ula = 0x9c;
	    for (var i=0; i<16; i++) {
		outpal[i] = i*0x10+toTTL(pal[i>>3]);
	    }
	    break;
	case 'bbcmicro.mode1.overscan':
	    ula = 0xd8;
	    for (var i=0; i<16; i++) {
		outpal[i] = i*0x10 + toTTL(pal[[0,0,1,1,0,0,1,1,2,2,3,3,2,2,3,3][i]]);
	    }
	    break;
	case 'bbcmicro.mode2.overscan':
	    ula = 0xf4;
	    outpal = new Uint8Array([0x07,0x16,0x25,0x34,0x43,0x52,0x61,0x70,
				     0x87,0x96,0xa5,0xb4,0xc3,0xd2,0xe1,0xf0]);
	    break;
	default: alert(dithertron.settings.id);
    }

    let req = { img: getNativeFormatData(),
		srcpng: dataURItoArrayBuffer(cropper.url),
		png: getImageFormat(),
		ula: ula,
		outpal: outpal
	      };
